...
// Usage: ./faultparser design.isc fault_description.txt
// Creates files faultdesign1.bench to faultdesignX.bench, where X is the number of faults
...
int parse (int argc, char * argv[]) {
  ...
  char *data;
  FILE *fp;
  int totalfaults = 0;  // one of these two needs to be passed on
  int fault_counter = 0;
  char c;
  ...
  if (!(fp3 = fopen(design_file,"r"))) { // just checks if design file exists
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {  // for each fault in the fault_description.txt file
    ...
    std::string gatedesignfile;
    gatedesignfile = std::string(argv[1]);
    filename2 = std::string(fault_array[fault_counter][0]);
    filename = filename1 + filename2 + filename3;
    std::ifstream src(gatedesignfile, std::ios::binary);
    ...
    if ((newgate == "SA0") && (stop == 1)) {
      outFile << "GATE1 = NOT(" << data5 << ")" << std::endl;
      outFile << "GATE2 = NXOR(GATE1, " << data5 << ")" << std::endl;
    } else if ((newgate == "SA1") && (stop == 1)) {
      outFile << "GATE1 = NOT(" << data5 << ")" << std::endl;
      outFile << "GATE2 = XOR(GATE1, " << data5 << ")" << std::endl;
    } else if ((newgate == "SA0") && (stop == 0)) {
      outFile << "GATE1 = NOT(" << lookfor << ")" << std::endl;
      outFile << "GATE2 = NXOR(GATE1, " << lookfor << ")" << std::endl;
    } else if ((newgate == "SA1") && (stop == 0)) {
      outFile << "GATE1 = NOT(" << lookfor << ")" << std::endl;
