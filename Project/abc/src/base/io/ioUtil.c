...
***********************************************************************/
Abc_Ntk_t * Io_ReadNetlist( char * pFileName, Io_FileType_t FileType, int fCheck, int var )
{
    FILE * pFile;
    Abc_Ntk_t * pNtk;
    /*
...
//        pNtk = Io_ReadBlif( pFileName, fCheck );
    ...
    */
    if ( FileType == IO_FILE_BENCH ) {
      pNtk = Io_ReadBench( pFileName, fCheck, var ); // might set var to 1 here
    } else {
...
***********************************************************************/
Abc_Ntk_t * Io_Read( char * pFileName, Io_FileType_t FileType, int fCheck, int fBarBufs, int var )
{
    Abc_Ntk_t * pNtk, * pTemp;
	Vec_Ptr_t * vLtl;
    // get the netlist
    pNtk = Io_ReadNetlist( pFileName, FileType, fCheck, var );
    if ( pNtk == NULL )
        return NULL;
...
Abc_Obj_t * Io_ReadCreateNode( Abc_Ntk_t * pNtk, char * pNameOut, char * pNamesIn[], int nInputs )
{
    ...
    //for (int j = 0; j<sizeof(pNamesIn); j++) {
    //  printf("pNamesIn[%d]: %s\n",j,pNamesIn[j]);
    //}
