...
static Abc_Ntk_t * Io_ReadBenchNetwork( Extra_FileReader_t * p, int var );
...
***********************************************************************/
Abc_Ntk_t * Io_ReadBench( char * pFileName, int fCheck, int var )
{
    ...
    if (var == 0) {
      ...
      // read the network
      pNtk = Io_ReadBenchNetwork( p, var );
...
***********************************************************************/
Abc_Ntk_t * Io_ReadBenchNetwork( Extra_FileReader_t * p, int var )
{
    ...
    */
    ...
    //pNtk = string_of_file_name, different one for each fault design
    ...
    pNtk = Abc_NtkStartRead( Extra_FileReaderGetFileName(p) ); // this is just the file name, so faultdesign1.bench etc
...
//                    Abc_ObjSetData( pNode, Abc_SopRegister(pNtk->pManFunc, "1-0 1\n-11 1\n") );
    ...
    return pNtk;
      ...
      printf("VAR = 1: IMPLEMENT READ FROM STRING HERE\n");
    }
