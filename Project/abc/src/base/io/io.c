...
int IoCommandRead( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( fReadGia )
    {
        Abc_Ntk_t * pNtk = Io_ReadNetlist( pFileName, Io_ReadFileType(pFileName), fCheck, 0 );
        if ( pNtk )
        {
    ...
    pNtk = Io_Read( pFileName, Io_ReadFileType(pFileName), fCheck, fBarBufs, 0 );
    if ( pNtk == NULL )
        return 0;
...
int IoCommandReadAiger( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_AIGER, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadBaf( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_BAF, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadBblif( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_BBLIF, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadBlif( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( fReadAsAig )
      ...
      pNtk = Io_Read( pFileName, IO_FILE_BLIF, fCheck, 0, 0 );
    else
    {
        Abc_Ntk_t * pTemp;
...
int IoCommandReadBlifMv( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_BLIFMV, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadBench( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_BENCH, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadEdif( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_EDIF, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadEqn( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_EQN, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadPla( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( fZeros || fBoth || fOnDc || fSkipPrepro )
    {
      ...
      pNtk = Io_Read( pFileName, IO_FILE_PLA, fCheck, 0, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadVerilog( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    pFileName = argv[globalUtilOptind];
    // read the file using the corresponding file reader
    pNtk = Io_Read( pFileName, IO_FILE_VERILOG, fCheck, fBarBufs, 0 );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
