...
int Abc_NtkComparePis( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int fComb )
{
    ...
    {
        printf("Writing outputs to a file");

        FILE *fp;
        fp = fopen("testOuts.txt","w+");
        fprintf(fp,"Networks have different number of primary inputs.\n");
        fclose(fp);
        printf( "Failure: Networks have different number of primary inputs.\n" );
        return 0;
